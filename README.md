...
`POST /ingest` validates the payload exactly as described in `json_scanboard.docx`:
- `ts` must be ISO-8601.
- `sys` describes the control system. Its `port` uniquely identifies records in MySQL.
